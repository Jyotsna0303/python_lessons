...
def main():
    print('connect')
    db = sqlite3.connect('db-api.db') #returns a database handle
    ...
    cur.execute("""
        ...
        """)
    print('commit')
    db.commit() #commit on db handler
    print('count')
    cur.execute("SELECT COUNT(*) FROM test")
    count = cur.fetchone()[0]
    print(f'there are {count} rows in the table.')
    print('read')
    for row in cur.execute("SELECT * FROM test"): #for with cur.execute()
        print(row)
    print('drop')
    cur.execute("DROP TABLE test")
    print('close')
    db.close() #close the handler()
